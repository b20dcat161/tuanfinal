  ...
  model.addAttribute("otherUserId", id);
    ...
    public String register(@RequestParam(value = "id", required = false) Integer id,
        ...
        if (id != null && "edit".equals(action)) {
            AppUser user = appUserDAO.getUserById(id); // Fetch user details
            if (user != null) {
                ...
                // Note: Do not set the password to avoid exposing it
            }
        }
    ...
    public String register(@PathVariable(value = "id",required = false) Integer id,
        ...
        try {
            ...
            if (id != null) {
                appUserDAO.updateUser(user); // Update existing user
            } else {
                appUserDAO.saveUser(user); // Add new user
            }
